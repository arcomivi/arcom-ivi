...
void ACIMedia::loadMedia() {
    ...
    if(m_iMediaType==MEDIA_INITIAL) {
        m_oMediaModel->addItem(new Item("MEDIA_MUSIC", "Music", "music", "folder"));
        m_oMediaModel->addItem(new Item("MEDIA_RADIO", "Radio", "radio", "folder"));
        m_oMediaModel->addItem(new Item("MEDIA_VIDEO", "Video", "video", "folder"));
        m_oMediaModel->addItem(new Item("MEDIA_PICTURE", "Pictures", "picture", "folder"));
        m_oMediaModel->addItem(new Item("MEDIA_TV", "TV", "tv", "folder"));
...
void ACIMedia::currentListIndexChanged(int listIndex) {
    ...
    if(m_iPreviousCurrentListIndex!=-1) {
        //TODO: m_oMediaModel->setData(m_oMediaModel->index(m_iPreviousCurrentListIndex),QVariant(""), ACIListModel::ValueRole2);
    }
    //TODO: refactor
...
void ACIMedia::displayInitialMusic() {
    m_oMediaModel->removeRows(0, m_oMediaModel->rowCount());
    m_oMediaModel->addItem(new Item("MEDIA_INITIAL", "..", "music", "folder"));
    m_oMediaModel->addItem(new Item("MUSIC_ALL_ALBUMS", "All Albums", "music", "folder"));
    m_oMediaModel->addItem(new Item("MUSIC_ALL_ARTISTS", "All Artists", "music", "folder"));
    m_oMediaModel->addItem(new Item("MUSIC_ALL_SONGS", "All Songs", "music", "folder"));
}
...
void ACIMedia::displayAllSongs() {
    ...
    m_oMediaModel->addItem(new Item("MEDIA_MUSIC","..","go_back_to_main", "left"));
        ...
        foreach (QFileInfo fileInfo, musicDir.entryInfoList(QDir::NoDotAndDotDot|QDir::AllEntries)) {
            ...
            if (fileInfo.isDir()) {
                ...
                foreach (QFileInfo song, QDir(songDir).entryInfoList(QDir::NoDotAndDotDot|QDir::AllEntries)) {
                    ...
                    m_oMediaModel->addItem(new Item("SONG", song.baseName(), song.absoluteFilePath()));
                ...
                m_oMediaModel->addItem(new Item("SONG", fileInfo.baseName(), fileInfo.absoluteFilePath()));
    ...
    if(m_iPreviousCurrentListIndex!=-1) {
        //        TODO: refactor m_oMediaModel->setData(m_oMediaModel->index(m_iPreviousCurrentListIndex),QVariant("black"), ACIListModel::ValueRole2);
    }
}
...
void ACIMedia::displayInitialVideo() {
    m_oMediaModel->removeRows(0, m_oMediaModel->rowCount());
    m_oMediaModel->addItem(new Item("MEDIA_INITIAL", "..", "music", "folder"));
    m_oMediaModel->addItem(new Item("VIDEO_ALL_VIDEOS", "All Videos", "video", "folder"));
}

void ACIMedia::displayAllVideos() {
    m_oMediaModel->removeRows(0, m_oMediaModel->rowCount());
    m_oMediaModel->addItem(new Item("MEDIA_VIDEO","..","go_back_to_main", "left"));
    m_videoList.clear();
        ...
        if (storage.isValid() && storage.isReady()) {
            ...
            if(videoDir.exists()) {
                ...
                foreach (QFileInfo videoFileInfo, QDir(videoDir).entryInfoList()) {
                    ...
                    m_oMediaModel->addItem(new Item("VIDEO", videoFileInfo.baseName(), videoFileInfo.absoluteFilePath()));
